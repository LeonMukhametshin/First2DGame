...
namespace DI
{
    public class DIContainer
    {
        ...
        public void RegisterInstance<T>(string tag, T instance)
        {
            ...
            {
                throw new Exception(
                $"DI: Factory with tag {key.Item1} and type {key.Item2.FullName} has already registration");
            }
        ...
        public T Resolve<T>(string tag = null)
        {
            ...
            if (_registrations.ContainsKey(key))
            {
                throw new Exception($"Cyclic dependency for find tag {tag} and type {key.Item2.FullName}");
            }
